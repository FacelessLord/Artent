...
public class MageEntityRenderer extends GeoEntityRenderer<MageEntity> {
    ...
    private final GeoHeldItemFeatureRenderer<MageEntity> heldItemRenderer;
...
//	}
    ...
    public void actuallyRender(
      MatrixStack poseStack, MageEntity animatable, BakedGeoModel model, RenderLayer renderType,
      VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick,
      int packedLight, int packedOverlay, float red, float green, float blue, float alpha
    ) {
        super.actuallyRender(poseStack,
                             animatable,
                             model,
                             renderType,
                             bufferSource,
        ...
        if (animatable.getPose() == EntityPose.SLEEPING) {
            ...
            if (bedDirection != null) {
                ...
                poseStack.translate(-bedDirection.getOffsetX() * eyePosOffset,
                                    0,
                                    -bedDirection.getOffsetZ() * eyePosOffset);
            }
        }
        ...
        if (!isReRender) {
            ...
            AnimationState<MageEntity> animationState = new AnimationState<>(animatable,
                                                                             limbSwing,
                                                                             limbSwingAmount,
                                                                             partialTick,
                                                                             avgVelocity >= motionThreshold &&
                                                                             limbSwingAmount != 0);
            ...
            animationState.setData(DataTickets.ENTITY_MODEL_DATA,
                                   new EntityModelData(shouldSit, animatable.isBaby(), -netHeadYaw, -headPitch));
