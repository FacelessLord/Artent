...
public class FermentingBarrel extends BlockWithEntity implements INamed {
		...
		if (stack.getItem() == ModItems.GoldenBucketFilled) {
			if (be.potionKey.equals("")) {
				...
				if (potion != null) {
					...
					player.setStackInHand(hand, ItemUsage.exchangeStack(stack, player, new ItemStack(ModItems.GoldenBucket)));
			...
			if (stack.getItem() == ModItems.SmallConcentratePhial) {
				...
				if (!player.isCreative())
					stack.decrement(1);
				var smallConcentrate = AlchemicalPotionUtil.setFermentedPotion(new ItemStack(ModItems.SmallConcentrate), potionKey);
				...
				if (amount == 0) return ActionResult.FAIL;
				var mediumConcentrate = AlchemicalPotionUtil.setFermentedPotion(new ItemStack(ModItems.MediumConcentrate), potionKey, amount);
				if (!player.isCreative())
					stack.decrement(1);
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return validateTicker(type, ModBlockEntities.FermentingBarrel, (world1, pos, state1, be) -> be.tick(world1, pos, state1));
	}
