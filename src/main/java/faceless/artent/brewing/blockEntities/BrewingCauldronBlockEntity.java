package faceless.artent.brewing.blockEntities;
...
public class BrewingCauldronBlockEntity extends BlockEntity {
	...
	public void tick(World world, BlockPos pos, BlockState state) {
		...
		if (state.get(BrewingCauldron.IS_BURNING)) {
			...
			if (waterAmount > 0) {
				var one16th = 1d / 16d;
				var box = new Box(pos.getX() + 2 * one16th, pos.getY() + 2 * one16th, pos.getZ() + 2 * one16th,
					pos.getX() + 1 - 2 * one16th, pos.getY() + 12 * one16th, pos.getZ() + 1 - 2 * one16th);
