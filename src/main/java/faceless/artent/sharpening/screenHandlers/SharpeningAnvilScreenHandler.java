...
public class SharpeningAnvilScreenHandler extends ScreenHandler {
    ...
    protected boolean canTakeOutput(PlayerEntity player, boolean present) {
        ...
        return !hammer.isEmpty() &&
               hammer.getItem() == ModItems.SmithingHammer &&
               !stack.isEmpty() &&
               stack.getItem() instanceof ISharpenable sharpenable
               &&
               (SharpeningAnvilInventory.isCatalyst(modifier.getItem()) &&
                SharpeningAnvilInventory.checkCatalysts(modifier, sharpenable.getLevel(stack)) ||
    ...
    public ItemStack quickMove(PlayerEntity player, int slotId) {
        ...
        if (slot.hasStack()) {
            ...
            if (slotId == 3) { // output
              ...
              ? !this.insertItem(slotStack, 2, 3, false) ||
                !this.insertItem(slotStack, 0, 1, false) : (item instanceof ISharpenable // insert tool into slot 0
              ? !this.insertItem(slotStack, 0, 1, false) : (item instanceof IEnhancer ||
                                                            Arrays
                                                              .stream(ModItems.Catalysts)
                                                              .anyMatch(cat -> cat == item)
              // insert catalyst or enchancer into slot 1
              ? !this.insertItem(slotStack, 1, 2, false) : (slotId >= 4 && slotId < 31 ? !this.insertItem(slotStack,
                                                                                                          31,
                                                                                                          40,
                                                                                                          false) :
              slotId >= 31 &&
              slotId < 40 &&
              !this.insertItem(slotStack, 3, 31, false)))))) {
                return ItemStack.EMPTY;
            }
    ...
    public SharpeningAnvilScreenHandler(
      int syncId, PlayerInventory inv, Inventory anvil, ScreenHandlerContext context
    ) {
        ...
        this.addSlot(new Slot(this.anvil, 1, 76, 47) {
            @Override
            public boolean canInsert(ItemStack stack) {
                return stack.getItem() instanceof IEnhancer ||
                       Arrays.stream(ModItems.Catalysts).anyMatch(cat -> cat == stack.getItem());
            }
        });
