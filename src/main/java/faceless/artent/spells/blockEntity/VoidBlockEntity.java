...
public class VoidBlockEntity extends BlockEntity {
    ...
    public void tick(World world, BlockPos pos, BlockState state) {
        ...
        if (ticksLeft <= 0) {
            var replacedBlock = NbtHelper.toBlockState(this.getWorld().createCommandRegistryWrapper(RegistryKeys.BLOCK),
                                                       replacedBlockTag);
        ...
        if (ticksLeft % 4 == 0) {
            ...
            if (depth <= 1 ||
                world.getBlockState(offsetPos).getBlock() == ModBlocks.VoidBlock ||
                !world.getBlockState(offsetPos).isOpaqueFullCube(world, offsetPos)) {
                depth = 0;
                return;
            }
