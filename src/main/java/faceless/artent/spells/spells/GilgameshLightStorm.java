    ...
    public SpellActionResult spellTick(ICaster caster, World world, ItemStack stack, int actionTime) {
        if (!world.isClient) {
            ...
            if (caster instanceof Entity entity) {
                var result = ProjectileUtil.raycast(entity,
                                                    start,
                                                    end,
                                                    new Box(start, end),
                                                    e -> true,
                                                    end.subtract(start).length());
                if (result != null) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                var spawnPos = projectileTargetPos.add(Math.cos(spawnAngle) * attackRange,
                                                       attackHeight,
                                                       Math.sin(spawnAngle) * attackRange);
