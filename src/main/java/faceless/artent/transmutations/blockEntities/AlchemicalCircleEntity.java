...
public class AlchemicalCircleEntity extends BlockEntity implements RenderDataBlockEntity {
    ...
    private void writeClientNbt(NbtCompound tag) {
        ...
        for (int i = 0; i < boundEntities.size(); i++) {
            ...
            elst.putIntArray(String.valueOf(i),
                             new int[]{entity.getPos().getX(), entity.getPos().getY(), entity.getPos().getZ(), entityDir.getRight().getId()});
        }
        ...
        if (transmutation != null) {
            if (state == State.Preparation) {
                ...
                return transmutation
                  .getActionColor()
                  .multiply(percentage)
                  .addNoMixing(transmutation.getPreparationColor().multiply(1 - percentage));
            }
